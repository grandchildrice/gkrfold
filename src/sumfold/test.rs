...
use super::*;
use ark_std::{rand::SeedableRng, rand::rngs::StdRng, One};
  ...
  for &n in &ns {
    for &l in &ls {
      for t in 0..num_tries {
        ...
        assert!(verified.is_err(), "Should failed, but succeed at n={} l={} t={}", n, l, t);

        // verify with invalid claim
        let mut prover_rng2 = StdRng::seed_from_u64(122);
        let rho_int = prover_rng2.gen_range(0..n);
        let invalid_q_poly = build_Q_polynomial(proof.Q_poly.evaluations[rho_int] + FF::one(), rho_int, (n as f64).log2() as usize);
